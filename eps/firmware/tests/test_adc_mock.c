...
void test_adc_read_write(void) {
    ...
    assert(adc_read(ADC_CHANNEL_0) == 1234);

    mock_adc_set_value(ADC_CHANNEL_3, 4321);
    assert(adc_read(ADC_CHANNEL_3) == 4321);

    // test overwrite
    mock_adc_set_value(ADC_CHANNEL_0, 555);
    assert(adc_read(ADC_CHANNEL_0) == 555);
...
void test_adc_out_of_bounds(void) {
    ...
    uint16_t value = adc_read(ADC_CHANNEL_MAX);
    assert(value == 0);
    ...
    value = adc_read(ADC_CHANNEL_MAX);
    assert(value == 0);
